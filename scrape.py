...
# ---------------------------------------------------------------------------
# UniPin: per-product URLs on /global/game/* with USD pricing.
# Same pattern as G2A/Kinguin/Eneba — render via Firecrawl or Patchright,
# then hand HTML to adapter.
# ---------------------------------------------------------------------------

async def scrape_unipin_dispatch(competitor_key: str, homepage: str, products: List[str],
                                 page: Optional["Page"], deadline: float) -> Dict:
    out: Dict = {}
    by_url: Dict[str, List[str]] = {}
    for p in products:
    ...
    for url, prods in by_url.items():
        ...
        if FIRECRAWL_KEY:
            try:
                ...
                print(f"    [UniPin] firecrawl fail: {e}")

        if not html and page:
            try:
                await page.goto(url, wait_until="domcontentloaded",
                                timeout=PLAYWRIGHT_NAV_TIMEOUT_MS)
                # UniPin renders denom-price tiles after initial load.
                try:
                    await page.wait_for_selector('.denom-price', timeout=8000)
                except Exception:
                    pass
                await page.wait_for_timeout(PLAYWRIGHT_WAIT_MS + 1000)
                html = await page.content()
            except Exception as e:
                print(f"    [UniPin] pw fail: {e}")

        if not html:
            print(f"    [UniPin] no rendered HTML for {url[-50:]}")
            continue

        prices = await adapters.scrape_unipin(homepage, prods, html=html)
        if not prices:
            html_lc = html.lower()
            print(f"    [UniPin] no tiles from {url[-40:]} html={len(html)} "
                  f"has_denom-price={'denom-price' in html_lc} "
                  f"has_USD={' usd ' in html_lc}")
        out.update(prices)
    return out
...
STATIC_COMPS = {"Seagm", "Codashop", "LapakGaming"}
...
def strategy_for(competitor_key: str) -> str:
    ...
    if _HAS_ADAPTERS and competitor_key == "Unipin":
        return "adapter_unipin"
...
async def main() -> int:
    ...
    for cat in categories.values():
        for k in (cat.get("competitors") or {}).keys():
            s = strategy_for(k)
            # adapter_g2a / adapter_kinguin / adapter_eneba / adapter_unipin
            # use Playwright as fallback when Firecrawl key isn't set.
            adapter_strategies = ("adapter_g2a", "adapter_kinguin", "adapter_eneba", "adapter_unipin")
    ...
    for comp_key, work in by_comp.items():
        ...
        for cat_name, homepage, products in work:
            ...
            try:
                ...
                elif strategy == "adapter_unipin":
                    prices = await scrape_unipin_dispatch(comp_key, homepage, products, page, deadline)
                else:
                    prices = await scrape_static(comp_key, homepage, products, deadline)
