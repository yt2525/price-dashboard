...
async def main() -> int:
    ...
    for cat in categories.values():
        for k in (cat.get("competitors") or {}).keys():
            s = strategy_for(k)
            # adapter_g2a / adapter_kinguin use Playwright when Firecrawl missing
            if s in ("playwright", "moogold") or (s in ("adapter_g2a", "adapter_kinguin") and not FIRECRAWL_KEY):
                needs_browser = True
                break
            # Eneba also uses Playwright fallback when no Firecrawl
