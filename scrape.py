...
# MooGold product pages have radio button per denomination. Clicking a
# radio updates a visible price display. We iterate through products in
# the same category page, click matching radio, read the price element.
# ---------------------------------------------------------------------------
...
async def scrape_moogold(competitor_key: str, homepage: str, products: List[str],
    ...
    for url, prods in by_url.items():
        ...
        for product in prods:
            ...
            if not denom:
                continue
            # MooGold uses labels like "PSN Card 25 USD" or "25 USD"
            label_variants = [
            ...
            for label in label_variants:
                try:
                    radio = page.locator(f"label:has-text('{label}'), span:has-text('{label}'), li:has-text('{label}')").first
            ...
            # Read the price — MooGold shows it in a .woocommerce-Price-amount or similar
            try:
                price_text = await page.locator(
                    ".woocommerce-Price-amount, p.price, .price ins .amount"
                ).first.text_content(timeout=2500)
            except Exception:
                price_text = ""
            val = parse_price(price_text or "")
            if val and 0.1 <= val <= 100000:
                out[product] = val
...
def strategy_for(competitor_key: str) -> str:
    if competitor_key == "MooGold":
        return "moogold"   # site-specific Playwright (radio clicks)
