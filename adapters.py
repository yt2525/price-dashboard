...
def _eneba_extract_pairs(html: str):
    ...
    Approach: parse the flat page text for tile-shaped patterns matching:
        Card N USD ... From $X.XX ... -N% $Y.YY
    Y.YY is the discounted selling price.

    Eneba uses randomized CSS-in-JS class names, so DOM-selector matching
    isn't reliable across deploys. The flat-text pattern is stable.
    """
    soup = BeautifulSoup(html, "html.parser")
    pairs: Dict = {}
    ...
    # Each tile is identifiable by "Card N USD ... From $X.XX ... $Y.YY".
    # We capture the denom (N) and the LAST $Y.YY that appears within ~150
    # chars of the denomination (which is the discounted price).
    # Pattern explanation:
    #   \b(?:Card|Pin|Gift\s*Card|Voucher|Top[\s-]*Up)?\s*
    #   (\d{1,5})\s*USD\b                           ← denomination
    #   .{0,200}?                                    ← any tile content (lazy)
    #   From\s*\$\s*\d+(?:\.\d{1,2})?\s*            ← face value
    #   -?\d+%\s*                                   ← discount tag
    #   \$\s*(\d+(?:\.\d{1,2}))                     ← actual selling price
    tile_re = re.compile(
    ...
    for m in tile_re.finditer(full):
        denom = float(m.group(1))
        price = float(m.group(2))
        # Filter: denomination must be a reasonable face value
        if denom < 1 or denom > 5000:
            continue
        # Sanity: price within 0.5× to 1.2× face value (Eneba is usually 5-15%
        # below face value because it's a marketplace).
        if price > denom * 1.2 or price < denom * 0.5:
            continue
        # First match per denom wins
        if denom not in pairs or pairs[denom] == "N/A":
            pairs[denom] = price

    # Optional N/A detection: if a denomination appears in the text but no
    # price follows within a tile-shaped window, AND a "no offers"-style
    # marker appears near it, mark it N/A. (Eneba doesn't usually surface
    # these in category listings — out-of-stock tiles are filtered out by
    # default — so this is mostly a safety net.)
