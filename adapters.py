...
# UniPin adapter
# ---------------------------------------------------------------------------
# UniPin has region-specific storefronts. The /global/ path is the US/global
# version with USD-denominated products. The MY/PH/ID etc storefronts use
# local currency.
#
# Tile DOM structure (verified in browser):
#   <div class="...">
#     <span>PSN 10 USD</span>       ← SKU label
#     <div class="...denom-price">USD 9.94</div>  ← selling price
#     <span>Off</span>              ← discount badge
#   </div>
#
# Flat text pattern (most reliable): "N USD USD Y.YY"
#   - First N: face value denomination
#   - Y.YY: actual selling price
#
# UniPin sells: PSN USD, Steam USA, Roblox USD, Valorant Points, Fortnite
# V-Bucks, Apex Coins, PUBG UC. NO iTunes/Apple/Amazon/Spotify/Razer in /global/.

async def scrape_unipin(homepage: str, products: List[str], html: str = "") -> Dict:
    """UniPin adapter. Caller must pass pre-rendered HTML (httpx is 403'd
    by Cloudflare). Parses /global/game/* product pages."""
    out: Dict = {}
    if not _HAS_BS4 or not html:
        return out
    ...
    by_url: Dict[str, List[str]] = {}
    for p in products:
        u = nav.derive_url("Unipin", homepage, p)
        if u:
            by_url.setdefault(u, []).append(p)

    denom_price_map = _unipin_extract_pairs(html)
    if not denom_price_map:
        print(f"    [UniPin] no USD denomination tiles parsed")
        return out
    print(f"    [UniPin] {len(denom_price_map)} USD denominations found")
...
def _unipin_extract_pairs(html: str):
    """Returns {denomination_numeric: float_price or 'N/A'}.

    UniPin tile text pattern: "{Brand} {N} USD USD {Y.YY} Off"
    where N is the face value denomination and Y.YY is the selling price.

    Pages are SSR-rendered so the text appears once in the HTML body.
    """
    soup = BeautifulSoup(html, "html.parser")
    pairs: Dict = {}
    ...
    # Strict pattern: "N USD USD Y.YY" — captures denom and selling price.
    # This is the dominant pattern on /global/game/playstation-psn-usd,
    # /global/game/steam-usa, /global/game/roblox-usd, etc.
    strict_re = re.compile(
        r"\b(\d{1,5})\s*USD\s+USD\s+(\d+(?:\.\d{1,2}))",
        re.I,
    )
    for m in strict_re.finditer(full):
        denom = float(m.group(1))
        ...
        if denom < 1 or denom > 5000:
            continue
        # UniPin Steam page applies ~+10% markup; PSN/Roblox slight discount.
        # Accept 0.5× to 1.2× face value.
        if price > denom * 1.2 or price < denom * 0.5:
            continue
        if denom not in pairs or pairs[denom] == "N/A":
            pairs[denom] = price

    # N/A detection: if a denomination appears in the text but no price
    # follows it within a tile-shaped window, mark it N/A if a sold-out
    # marker is nearby.
    if not pairs:
        # Try the looser variant — some tiles may just have "USD" prefix
        # missing one side of the double "USD USD" repetition.
        loose_re = re.compile(
            r"\b(\d{1,5})\b[\s\S]{0,80}?USD[\s\S]{0,40}?(\d+(?:\.\d{1,2}))",
            re.I,
        )
        for m in loose_re.finditer(full):
            denom = float(m.group(1))
